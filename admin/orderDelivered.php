...
//  redirect("index.php");
 ...
 $data=$_GET['ref'];
$values=functionforDeliveryDatabase($conn,$data);
...
$area=$values['area'];
	...
	$stmtinsert=$conn->prepare("INSERT INTO tb_deliveredorder (`id`,`productname`,`product_id`,`productquantity`,`price`,`order_by`,`fname`,`lname`,`phone`,`state`,`municipality`,`city`,`area`) VALUES (:id,:productname,:product_id,:productquantity,:price,:order_by,:fname, :lname, :phone, :state,:municipality,:city,:area)");
...
$stmtdelete=$conn->prepare("DELETE FROM tb_confirmorder WHERE id=:id");
